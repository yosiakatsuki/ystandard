...
yStandard は `$args` に `defer` を渡していないため、その notice には該当しません。一方で `wp_script_add_data( $handle, 'defer', true )` と `script_loader_tag` フィルターで属性を追加する独自実装を持っていました。

WP70-10 では `Enqueue_Utility::add_defer()` / `add_async()` で、WordPress 6.3 以降かつ alias ではない script handle に Core の `strategy` を設定するように変更しました。WordPress 6.1 / 6.2 互換のため、既存の `defer` / `async` データも残しています。Core が `strategy` を処理する場合、`script_loader_tag` 側では `async` / `defer` を追加しないため、二重属性や Core の依存関係解決との衝突を避けます。
