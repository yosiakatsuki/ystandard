...
- フックプレフィックス: `ys_`
- 配布テーマとしての必須 PHP: 7.4+
- 開発環境の PHP: composer.json の要件に従う
- 必須 WP: 6.1+
...
- 個人環境に依存する設定（`.codex/`, MCP 設定, Serena 設定など）はリポジトリに含めない。
- シンボル構造の把握には、利用可能であれば Serena などのコード理解ツールを使う。
- 変更後は影響範囲に応じて `npm run lint`, `npm run build`, `npm run test` のいずれかを実行する。実行できない場合は理由を報告する。
