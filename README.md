...
- [修正] 「ボタンの縦・横サイズをあわせる」が無効な場合に、ページ先頭へ戻るボタンが動作しない不具合を修正

### v4.59.0 : 2026/08/22
- [調整] WordPress 7.1対応
- [調整] 投稿設定・SEO設定・SNS設定をブロックエディターの設定パネルへ移行
- [調整] `[ys]パーツ`のショートコード表示・コピー機能をブロックエディターの設定パネルへ移行
- [追加] サイトエディター向けコアブロックを有効化する上級者向け設定を追加

### v4.58.0 : 2026/08/04
- [調整] ブロック用CSSの読み込み順序の調整

### v4.57.0 : 2026/05/14
- [調整] WordPress 7.0向け調整
- [追加] カスタマイザーのフォント設定に「外観」→「フォント」で追加したフォントを選択できるように調整

### v4.56.2 : 2026/04/06
- [調整] テーマアップデートに使用しているライブラリのアップデート

### v4.56.1 : 2026/04/01
- [修正] アンカー付き外部リンクが新しいタブで開かない不具合を修正

### v4.56.0 : 2026/03/04
- [追加] モバイルフッターメニューに余白設定追加

### v4.55.0 : 2025/12/04
- [調整] WordPress 6.9向け調整
- [調整] 投稿一覧などの抜粋表示に埋め込みブロックの内容(URL)が含まれないように調整
- [追加] アコーディオンブロック用スタイル追加
- [修正] メタボックスCSS不具合対処

### v4.54.3 : 2025/09/18

- [修正] グローバルナビゲーション開閉時のクラス追加・削除処理不具合修正

### v4.54.2 : 2025/08/22

- [修正] プロフィール画像が縦に伸びる事がある不具合の修正（管理画面側）

### v4.54.1 : 2025/08/21

- [修正] プロフィール画像が縦に伸びる事がある不具合の修正
- [調整] プロフィール画像を正方形で表示するように調整

### v4.54.0 : 2025/07/08

- [修正] SNSシェアボタン機能からPocketを削除

### v4.53.1 : 2025/05/20

- [修正] ドロワーメニューボタンの余白とサイトタイトルの余白の切り替わりタイミングが合わない点の修正
- [修正] メニュー編集ページのマニュアルリンク不具合修正

### v4.53.0 : 2025/04/18

- [調整] WordPress 6.8向け調整
- [修正] アイコンリストからLinkedinを削除

### v4.52.0 : 2025/01/07

- [修正] Facebook公式シェアボタン不具合対応…いいねボタンからシェアボタンへ変更
- [追加] `ys_get_header_thumbnail` フック追加
...
- [調整] WordPress 6.7向け調整
- [修正] SNSシェアボタンのBlueskyの初期値不具合修正

### v4.50.1 : 2024/07/31

- [修正] スクリーンリーダーテキスト関連の不具合修正
- [調整] 検索モーダル画面のタブ遷移調整
- [調整] グローバルメニューのタブ遷移調整
- [調整] aria-label設定（モバイルメニュー開閉ボタン、ページネーション、記事一覧ショートコードサムネイル等）

### v4.50.0 : 2024/07/17

- [調整] WordPress 6.6向け調整
- [追加] SNSシェアボタンにBlueskyを追加
...
- [調整] 新着記事一覧表示ショートコードのカテゴリー表記のターム用クラスをcategory--[タームslug]から[タクソノミーSlug]--[タームSlug]に変更
...
- [追加] ターム・タクソノミー表示に関するフックの追加
    - ys*get*${post_type}\_archive_category_terms_length
    - ys*get*${taxonomy}\_archive_taxonomy
    - ys*get*${post_type}\_archive_taxonomy
