...
class Blocks_Test extends WP_UnitTestCase {
	...
	public function test_enqueue_block_style_after_global_styles_and_theme_styles() {
		...
		$global_style_handle = wp_style_is( 'wp-global-styles-placeholder', 'enqueued' )
			? 'wp-global-styles-placeholder'
			: 'global-styles';
		$block_style_handle  = $this->get_button_style_handle();
		$global_position     = array_search( $global_style_handle, wp_styles()->queue, true );
		$theme_position      = array_search( 'ystandard', wp_styles()->queue, true );
		$block_position      = array_search( $block_style_handle, wp_styles()->queue, true );

		$this->assertNotFalse( $global_position );
		$this->assertNotFalse( $theme_position );
		$this->assertNotFalse( $block_position );
		$this->assertGreaterThan( $global_position, $theme_position );
		$this->assertGreaterThan( $theme_position, $block_position );
		$this->assertSame( [], wp_styles()->registered[ $block_style_handle ]->deps );
	}
