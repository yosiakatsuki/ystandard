...
class Customizer_Controls {
	 ...
	 */
	public function set_section( $setting_name, $section_name ) {
		// セクションを移動させたいときに使う.
		$this->wp_customize->get_control( $setting_name )->section = $section_name;
	}
	 ...
	 */
	public function set_section_description( $name, $description ) {
		$this->wp_customize->get_section( $name )->description = $description;
	}
	...
	public static function sanitize_checkbox( $value, $setting ) {
		...
		return isset( $value ) && Convert::to_bool( $value ) ? true : $default;
	}
	 ...
	 * @return int
	 */
	public static function sanitize_number( $number, $setting, $allow_empty = true ) {
		// 入力値が数値でない場合はデフォルト値を返す.
		if ( ! is_numeric( $number ) ) {
			// 空文字の場合、空文字OKなら空文字、NGならデフォルト値を返す.
			return $allow_empty && '' === $number ? '' : $setting->default;
		}
	...
	public static function sanitize_plain_text( $value ) {
		$value = Text::get_plain_text( $value );
